...
| T-6 | `TradierClient` can't be instantiated — it's abstract (missing `supports_options/supports_equities/place_equity_order/get_bars/get_latest_quote` from `BrokerInterface`) | ✅ | **Removed** the dead class + its test (never wired into `broker_factory`; no Tradier account). Also fixed `.env.prod.example` which defaulted to the invalid `BROKER=tradier` (would `ValueError`). Kept `tradier_api_key`/`tradier_sandbox` — `paper_trader` still uses them for the optional live-**quote** feed |
